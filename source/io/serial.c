...
void l_serial_device_destroy (LSerialDevice *dev)
{
...
void l_serial_set_access_mode (LSerialDevice *serial_device, LAccessMode access)
{
	if ( *serial_device )
	{
		...
		{
			case L_READ_ONLY:
				(*serial_device)->access = L_READ_ONLY;
				break;
			case L_WRITE_ONLY:
				(*serial_device)->access = L_WRITE_ONLY;
				break;
			case L_READ_WRITE:
				(*serial_device)->access = L_READ_WRITE;
				break;
			default:
				(*serial_device)->access = L_ACCESS_NONE;
		}
...
void l_serial_action (int signum, siginfo_t *info, void *context)
{
	...
	if ( info )
	{
		for ( int i = 0; i <= arrlen(l_serial_device_list); i++)
		{
			...
			{
				l_serial_receive_irq_callback(&l_serial_device_list[i]);
			}
		}
	}
}
